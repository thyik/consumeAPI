...
class OcbcApi extends Component {
    ...
    render() { 
        return ( <section>                
            ...
            <button className="App-button" onClick={this.fetchNewCarFinancing}>
                New Car Financing
            </button>
            ...
            <p>{this.state.resData}</p></section> );
    }
    ...
    fetchSaving = () => {
        fetch(
            ...
            .then((res) => res.json())
            .then(
                (data) => this.setState({resData : JSON.stringify(data)})
            )
            .catch((err) => {
                console.log(err);
                return err;
    ...
    fetchForex = () => {
        fetch("https://api.ocbc.com:8243/Forex/1.1", {
            ...
            .then(
                (data) =>
                    this.setState({resData : JSON.stringify(data)})
            )
            .catch((err) => {
                console.log(err);
                return err;
